...
public class PlayerSnake {
	...
	public PlayerSnake(MainWindow GameUI,Foodset food,Obstacle obstacle){
		...
		for(int i = 0;i < 4;i++)
		{
		    String s1="head//head" + i + ".png";
            //System.out.println(s1);
			headIcon[i] = new ImageIcon(s1);
			headIcon[i].setImage(headIcon[i].getImage().getScaledInstance(20,20,Image.SCALE_SMOOTH));//保持图片的清晰
            String s2="body//body" + i + ".png";
			bodyIcon[i] = new ImageIcon(s2);
	...
	public void move(){
		...
		if(checkDeath(next_coor))//判断下一步是否死亡
		{
			...
			//System.out.println(point+"分");;
			int a=Write2file.readPoint("sorce.txt");
			if (point > a){
				//System.out.println("最高分："+a+point);
				Write2file.writePoint(point,"sorce.txt");
			}
			int result=JOptionPane.showConfirmDialog(null, "大侠请重新再来",
		...
		else
		{
			...
			next_node.label.setVisible(true);//根据坐标显示图片
