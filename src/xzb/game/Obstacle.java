...
public class Obstacle {
		...
		for(int i = 0;i < wallAmount;i++)
		{
			...
			if(tag == 0)
			{
				...
				for(int j = 0;j < length;j++)
				{
					...
					wall.wall.add(brick);//把该块砖添加到该堵墙中去 二维数组
				}
	...
	public boolean checkSafe(Coordinate coor,int tag,int length)//坐标 墙的方向，长度
	{
		if(tag == 0)//表示墙的横向
		{
			for(int i = 0;i < length;i++)
			{
				if(GameUI.getMap()[coor.x][coor.y + i] != 0)//当横向不为0，返回不可创建[coor.y + i]横向
					return false;
			}
		}
		else if(tag == 1)
	...
	public synchronized void removeAll(){//移除界面上的所有砖块图片
		for (Iterator<Wall> iter = obstacle.iterator(); iter.hasNext();) {
			...
			for (Iterator<Brick> iter2 = _wall.wall.iterator(); iter2.hasNext();) {
				...
				//_wall.wall.clear();//若如此设置则只执行一次
			}
		}
		obstacle.clear();//在for外面清除
	}
	//移除界面上的一张砖块图片shitf
